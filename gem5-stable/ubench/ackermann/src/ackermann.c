...
ack(unsigned x, unsigned y){
	...
	depth++;
	if(depth > MAX_DEPTH){
		cosim_printf("Maximum stack depth %d exceeded. Abort.\n", MAX_DEPTH);
		cosim_success();
	}
	if( x >= MAX_X) {
		cosim_printf("Maximum x value %d exceeded. Abort. \n", MAX_X);
		cosim_success();
	}
	if( y >= MAX_Y) {
		cosim_printf("Maximum y value %d exceeded. Abort. \n", MAX_Y);
		cosim_success();
	}
...
main(void)
{
	...
	while( (i<argc) && (argv[i][0] == '-')){
		  ...
		  if(strcmp(argv[i],"-r")==0){
			  ...
			  cosim_printf("\ta(x,0)     = x + 1\n");
			  cosim_printf("\ta(0,y+1)   = a(1,y)\n");
			  cosim_printf("\ta(x+1,y+1) = a(a(x,y+1),y)\n\n"); 
			  cosim_printf("where x and y are nonnegative integers.\n");
			  cosim_printf("It is known that a is recursive, but not primitive recursive.\n\n");
			  cosim_success();
	          }
		  cosim_printf("ackermann: Unknown option %s\n", argv[i]);
		  cosim_printf("%s\n",USAGE);
		  return 1;
	}
